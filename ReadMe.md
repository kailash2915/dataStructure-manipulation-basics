**Array Questions**

1. Find the maximum and minimum element in an array. - Y
2. Find the second largest element in an array. - Y
3. Reverse an array. - Y
4. Remove duplicates from an array. - Y
5. Find the missing number in an array from 1 to N. - Y
6. Find all duplicate elements in an array.
7. Move all zeros in an array to the end.
8. Rotate an array to the right by K steps.
9. Merge two sorted arrays into a single sorted array.
10. Find the intersection of two arrays.
11. Find the union of two arrays.
12. Find the majority element in an array.
13. Find the subarray with maximum sum (Kadane’s algorithm).
14. Check if an array is sorted.
15. Find the first repeating element in an array.


**Integer / Number Questions**

1. Reverse an integer.
2. Check if a number is a palindrome.
3. Count the number of digits in an integer.
4. Find the factorial of a number.
5. Check if a number is prime.
6. Find the greatest common divisor (GCD) of two numbers.
7. Find the least common multiple (LCM) of two numbers.
8. Check if a number is an Armstrong number.
9. Check if a number is even or odd.
10. Find the sum of digits of a number.
11. Find the product of digits of a number.
12. Print Fibonacci series up to N numbers.


**String Manipulation Questions**

1. Reverse a string.
2. Check if a string is a palindrome.
3. Count the frequency of each character in a string.
4. Check if two strings are anagrams.
5. Find the first non-repeating character in a string.
6. Remove duplicate characters from a string.
7. Count vowels and consonants in a string.
8. Convert a string to uppercase without using built-in methods.
9. Reverse the words in a sentence.
10. Find the longest word in a sentence.
11. Check if a string contains only digits.
12. Find the number of words in a sentence.
13. Count character frequency in a string and print in ascending order
14. Print all substrings of a given string


**Mixed Array & String Logic Questions**

1. Find the longest substring without repeating characters.
2. Find the longest common prefix among strings.
3. Group anagrams from a list of strings.
4. Find the longest palindrome substring in a string.
5. Implement string compression (e.g., "aaabb" → "a3b2").
